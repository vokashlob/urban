...
def add_to_blocked(input_id):
    cursor.execute(f'UPDATE Users SET block = 1 WHERE user_id = {input_id}')
    connection.commit()


def remove_from_blocked(input_id):
    cursor.execute(f'UPDATE Users SET block = 0 WHERE user_id = {input_id}')
    connection.commit()


def check_block(user_id):
    users = cursor.execute(f'SELECT block FROM Users WHERE user_id = {user_id}').fetchone()
    connection.commit()
    return users[0]
